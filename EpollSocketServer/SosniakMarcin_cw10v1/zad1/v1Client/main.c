...
void initUnix(char* sMyName,char* sUnixSocketName)
{
    ...
    int mySockFd=socket(AF_UNIX,SOCK_STREAM ,0);
...
void live(char* sMyName, int mySockFd)
{
    ...
    if(msg.m_type==DECLINED)
    {
        ...
        iRet=shutdown(mySockFd,SHUT_RDWR);
        if(iRet<0){perror("error on shutdown");}
    ...
    iRet=shutdown(mySockFd,SHUT_RDWR);
    if(iRet<0){perror("error on shutdown"); iErrors++;}
